# AI/KE – Raport (Lista 1, Zadanie 1)

Autorzy: Michał Karol, Paweł Narolski, Piotr Syga  
Data zadania: 27 lutego 2026
...
Uwaga: to podejście jest heurystyczne (ranking tras względem „liczby przesiadek” realizowany jest przez dominującą karę, a nie przez bezpośrednie zliczanie przesiadek).
...
**Tło teoretyczne:** Tabu Search jest metaheurystyką, która iteracyjnie bada sąsiedztwo bieżącego rozwiązania, ale unika cykli dzięki pamięci tablicy tabu (zbiór ruchów/rozwiązań niedozwolonych). Może również używać „aspiracji”, aby mimo tabu dopuścić rozwiązania wystarczająco dobre.
...
Punkt (a) został zrealizowany poprzez implementację struktury listy Tabu zapobiegającej cyklom. Kolejne punkty (b-d) stanowią jego rozwinięcie zgodnie z wymaganiami zadania.
...
- na `stdout` wypisywana jest trasa (dla każdej części: start przystanku, linia, odjazd, przyjazd),
- na `stderr` wypisywane są metryki (czas obliczeń, liczba odwiedzonych stanów, wartość kryterium).
...
```bash
/home/dawid/Pulpit/artificial-intelligence-and-knowledge-engineering/task-1/venv/bin/python main.py "Wrocław Główny" "Legnica" t 06:30:00 20260303 dijkstra
```
Wynik (z `stderr`): odwiedzone stany `37689`, czas `0.6226s`, wartość kryterium `3540`.

Przypadek 2: `A*`, `t`, `Wrocław Główny -> Legnica`
```bash
/home/dawid/Pulpit/artificial-intelligence-and-knowledge-engineering/task-1/venv/bin/python main.py "Wrocław Główny" "Legnica" t 06:30:00 20260303 astar
```
Wynik (z `stderr`): odwiedzone stany `129`, czas `0.0096s`, wartość kryterium `3540`.

Przypadek 3: `Weighted A*`, `t`, `Wrocław Główny -> Legnica`
```bash
/home/dawid/Pulpit/artificial-intelligence-and-knowledge-engineering/task-1/venv/bin/python main.py "Wrocław Główny" "Legnica" t 06:30:00 20260303 weighted_astar
```
Wynik (z `stderr`): odwiedzone stany `13`, czas `0.0018s`, wartość kryterium `6420` (gorsze od wariantu nie-zwagowego).

Przypadek 4: `A*`, `p`, `Wrocław Główny -> Legnica`
```bash
/home/dawid/Pulpit/artificial-intelligence-and-knowledge-engineering/task-1/venv/bin/python main.py "Wrocław Główny" "Legnica" p 06:30:00 20260303 astar
```
Wynik (z `stderr`): odwiedzone stany `297`, czas `0.0112s`, wartość kryterium `3540`.

Przypadek 5: `Tabu Search` (TSP po liście), `t`, `Legnica -> Jelenia Góra;Głogów`
```bash
/home/dawid/Pulpit/artificial-intelligence-and-knowledge-engineering/task-1/venv/bin/python main.py "Legnica" "Jelenia Góra;Głogów" t 06:30:00 20260303 astar
```
Wynik (z `stderr`): czas `1.1538s`, łączny koszt `(t)` `33600`.
...
- w gałęzi TSP algorytm wyszukiwania najkrótszych ścieżek między kolejnymi parami `(u, v)` jest ustalany jako `weighted_astar` (zmienna z argumentu CLI nie wpływa na TSP).
...
4. Tabu Search bywa wolne, bo dla każdej modyfikacji permutacji liczyłem sporo odcinków (w praktyce wiele razy odpalałem wyszukiwanie najkrótszej ścieżki). Żeby to ugryźć, ograniczyłem sąsiedztwo (próbkowanie do max 10 kandydatów) i rozmiar tabu w wersji FIFO.

5. Do raportu udało mi się wstawić prawdziwe wyniki metryk, bo uruchomienia robiłem w `task-1/venv` (żeby była biblioteka `pandas`).
